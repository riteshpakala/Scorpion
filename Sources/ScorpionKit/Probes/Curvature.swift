...
//  diag J comes from Hutchinson probes (E[v ⊙ Jᵀv] = diag J for Rademacher v) via one VJP
//  each; backends without autodiff fall back to the forward-only score difference
//  (ε̂_a − ε̂_b)², the paper's Fisher-identity surrogate.
//
...
public enum Curvature {
    ...
    /// Hutchinson estimate of diag(∂ε̂/∂x) at `x` (B, …latent). Rows are independent, so one
    /// VJP of the batch sum serves every row. Probes with the same `index` are shared across
    /// calls — pass the same index for target and base to pair them.
    public static func diagJacobian(_ backend: DiffusionBackend, x: MLXArray, logSNR: MLXArray,
                                    conditions: [Conditioning], probes: Int, schedule: SeedSchedule,
                                    index: Int) -> MLXArray {
        var acc = MLXArray.zeros(x.shape)
        for k in 0..<max(probes, 1) {
            let v = rademacher(schedule, index &* 1_000 &+ k, shape: x.shape)
            let f: (MLXArray) -> MLXArray = { xx in
                (v * backend.predictEpsilon(xx, logSNR: logSNR, conditions: conditions)).sum()
            }
            acc = acc + v * grad(f)(x)
            eval(acc)
        }
        return acc / Float(max(probes, 1))
    }

    /// Exact diag(∂ε̂/∂x) for one point `x` (1, …latent) by one VJP per coordinate, batched.
    /// Small latents only (tests and ground truth).
    public static func exactDiagJacobian(_ backend: DiffusionBackend, x: MLXArray, logSNR: Float,
