...
//  ScorpionKit
//
//  Seed → latent for any DiffusionBackend, in log-SNR λ: DDIM (η = 0) or DPM-Solver++(2M)
...
public struct DeterministicSampler: Codable, Sendable {
    ...
    func scales(_ backend: DiffusionBackend, _ lambda: Float) -> (Float, Float) {
    ...
    func epsilon(_ backend: DiffusionBackend, _ x: MLXArray, lambda: Float, conditions: [Conditioning],
    ...
    public func sample(_ backend: DiffusionBackend, noise z: MLXArray, conditions: [Conditioning],
    ...
    public func invert(_ backend: DiffusionBackend, x0: MLXArray, conditions: [Conditioning],
